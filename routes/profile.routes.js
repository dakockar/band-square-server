...
router.get('/musician-profile/:userId', (req, res) => {
  MusicianModel.findById(req.params.userId)
    .then((response) => {
...
router.patch('/musician-profile/:id', (req, res) => {
  ...
  MusicianModel.findByIdAndUpdate(id, { $set: { firstName, lastName, location, instrument, bandName, genre, aboutMe, imgUrl } }, { new: true })
    .then((response) => {
